...
8. **Silence by default.** All sound (bible §14) ships muted with one visible toggle, preference persisted. Ambience loads lazily on first unmute. This deliberately amends the bible's Sound section from "subtle" to "opt-in subtle"; logged as a standing decision.
...
- Scene art ≤ 700KB total compressed at the largest breakpoint; the entry screen covers the fetch: a framed cartoon portrait of Sowan (asset V-034) with welcome text and a "press Enter / tap to come in" affordance rendered in DOM (law 3). The affordance enables when the scene is ready; the standard-view link stays visible on this screen (law 5). Entering plays a warm CRT power-on transition — cream/amber phosphor bloom, ≤ 1s, no strobing, never a cold glitch (art_style.md §8) — then the office fades in; `prefers-reduced-motion` replaces the transition with a ≤ 0.15s crossfade. LCP < 2.5s on mid-range mobile 4G; CLS < 0.05.
